...
int main()
{
    ...
    // to hold the disks
    src = cria(num_of_disks);
    aux = cria(num_of_disks);
    dest = cria(num_of_disks);
...
{ 	
    ...
    if (desempilha(&pole1TopDisk,src)==FRACASSO)//;pole1TopDisk == INT_MIN)
    {
    ...
    else if (desempilha(&pole2TopDisk,dest)==FRACASSO) //;pole2TopDisk == INT_MIN)
    {
